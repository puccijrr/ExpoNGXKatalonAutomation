...
//***attempt to*** Navigate to URL to Permission Bundle Sponsor edit on NGX
WebUI.navigateToUrl(GlobalVariable.permissionBundleURL)

WebUI.delay(7)

//Ensure that URL is REDIRECTED to event home
check_link() //WebUI.closeBrowser()
...
def check_link() {
    ...
    if (url_check == GlobalVariable.eventHomeURL) {
        KeywordUtil.markPassed('PASS: the expected redirect URL was found (event home)')

        log.logInfo(url_check)
    } else {
        KeywordUtil.markFailed('FAIL: incorrect url found, may NOT have been redirected to correct location of event home!!')
