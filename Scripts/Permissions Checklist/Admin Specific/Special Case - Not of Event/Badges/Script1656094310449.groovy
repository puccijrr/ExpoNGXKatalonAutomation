...
//Navigate to URL to Badge Editor page
WebUI.navigateToUrl(GlobalVariable.badgesURL)

WebUI.delay(3)

//Ensure that URL is still correct on event home (did NOT direct to event badge settings)
check_link()
...
def check_link() {
    ...
    if (url_check == GlobalVariable.events) {
        KeywordUtil.markPassed('PASS: the expected redirect URL was found')
